...
GameRet sm_handler()
{
  ...
  default:
    log_message(LOG_ERROR,
                "sm_handler(): hit non-functioning state: %s",
                log_enum_to_str_states(sm_state));
...
GameRet sm_state_next_get(Events ev)
{
  log_message(LOG_DEBUG,
              "sm_state_next_get(): current state: %s",
              log_enum_to_str_states(sm_state));
  log_message(LOG_DEBUG,
              "sm_state_next_get(): current event: %s",
              log_enum_to_str_events(ev));

  /* Transition to next state */
  sm_state = sm_state_transition[sm_state][ev];

  log_message(LOG_DEBUG,
              "sm_state_next_get(): next state   : %s",
              log_enum_to_str_states(sm_state));
...
static GameRet sm_handle_empty_board()
{
  ...
  {
    log_message(LOG_ERROR,
                "failed sm_handle_empty_board():event_next_event_set()");
    return ret;
  }
...
static GameRet sm_handle_player_turn()
{
  ...
  while (true)
  {
    ...
    {
      log_message(LOG_ERROR,
                  "Failed sm_handle_player_turn():render_gameboard()");
      return ret;
    };
    ...
    {
      log_message(LOG_ERROR,
                  "Failed sm_handle_player_turn():gamelogic_player_turn_get()");
      return ret;
    }
    ...
    if (row < 0 || row > 2 || col < 0 || col > 2)
    {
      ...
      log_message(
          LOG_WARN,
          "sm_handle_player_turn(): player enters above boundary row:%d col:%d",
          row,
          col);
      sleep(2);
      continue;
    ...
    if (!gamelogic_board_validate_empty_cell(row, col))
    {
      ...
      log_message(
          LOG_WARN,
          "sm_handle_player_turn(): player enters occupied cell row:%d col:%d",
          row,
          col);
      sleep(2);
      continue;
  ...
  {
    log_message(LOG_ERROR,
                "failed sm_handle_player_turn():event_next_event_set()");
    return ret;
  }
...
static GameRet sm_handle_check_win()
{
  ...
  if (found_winner)
  {
    ...
    {
      log_message(LOG_ERROR,
                  "failed sm_handle_check_win():event_next_event_set()");
      return ret;
    }
  }
  else
